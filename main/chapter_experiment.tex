...
It comprises two rings that contain and accelerate counter-rotating bunches
of particles.
...
% LHC
\item Finally, protons from the SPS are injected into the LHC, which finishes
...
resonates with a specially designed cavity, thus producing strong oscillating
...
% "Give number of magnets" - internal examiner
The LHC includes
$1232$ dipole magnets
and $4800$ higher-pole `corrector' magnets~\cite{lhc2008machine}.
...
% size of beam
At its peak luminosity by design specifications, each bunch of an LHC beam
...
The relative uncertainty on \atlas's Run~2 integrated luminosity,
in the form that we use in the $\twoljets$ search,
is $\pm1.7\%$~\cite{atlas2022searches,ATLAS-CONF-2019-021}.
More recent measurements have reduced this uncertainty to
$\pm0.83\%$~\cite{ATLAS:2022hro}.
...
illusion of missing transverse momentum.
...
Section~\ref{sec:atlas_object_resolutions}.
...
momentum by associating nearby photon-induced calorimeter clusters back to
their source electron many cases~\cite{atlas1994proposal, atlas1999design1}.
...
$b_3 \approx 0.03$.
With these parameters, this formula approximately describes the measured
relative jet resolutions of
...
which are highlighted in \textbf{bold}.
Rates overlap between triggers.
This table is adapted from~\cite{ATL-DAQ-PUB-2019-001}.
}
