...
A TTree resembles a columnar collection of named (jagged) arrays,
and is entirely unlike the `tree' structures of mathematics, computer
science, and dendrology.
}),
...
\label{sec:searches_heplikelihood}
...
At temperature $1/k_B$, this can induce a probability distribution
...
model
(for example, I recommend trying SR-HM(disc.) of
$1\ell bb$~\cite{SUSY-2019-08,hepdata.90607})
...
The formula for a standard \roofit\ \heplikelihood\ function can be
summarised as
\begin{equation}
\label{eqn:searches_heplikelihood_e}
e^{E(x)} =
\prod_{b \in \mathrm{bins}}\mathrm{Poisson}(n_b \mid y_b(x))
\times \prod_{i \in \mathrm{parameters}} f_i(x_i),
\end{equation}
in which $\mathrm{Poisson}(n\mid \mu)$ is the Poisson likelihood function
of Equation~\ref{eqn:searches_poisson}, $y_b(x)$ is the yield in bin $b$
for parameters $x$, and $f_i$ is the constraint function for parameter $i$.
Constraint functions $f_i$ are typically of
Gaussian (Equation~\ref{eqn:searches_gaussian}) or
Poissonian (Equation~\ref{eqn:searches_poisson} or a continuous interpolation)
forms, and often interpreted as likelihoods on fictional data.
The mappings from parameters to yields $y_b(x)$ are parameterised to
non-linearly interpolate between given `low', `central', and `high' variations
of the yields corresponding to certain parameters; many such
interpolation/extrapolation functions are available, but the most common
interpolates with a sixth-order polynomial that is smoothly attached to
exponential tails for extrapolation.
To avoid the numerical difficulties associated with taking a product over many
terms, one typically handles $E(x)$, not $e^{E(x)}$.
The bins may be further grouped into channels that share
terms~\cite{verkerke2003roofit}.
...
Each search has its own desiderata, so many different criteria can be used
to choose these logical selections; broadly, however, selections are made to
separate signals from backgrounds, such that data appearing in the regions are
informative as to whether signals are present and what the values of unknown
parameters might best explain the data.
...
Control regions are included in all fits with an intent to constrain the
normalization of Standard Model background samples.
Validation regions are not directly included as likelihood constraints in any
fits, but are used for visual inspection only, such that the analysts can
iteratively adapt their modelling to look good in validation regions without
directly biasing final results.
Signal regions are included only once the backgrounds are constrained and
validated and only in fits where one is testing for the presence of signals
as detailed below.
...
These fits can use multiple signal regions and control regions simultaneously,
and couple the effect of parametrized modifications to the signals and
backgrounds between the bins of those regions.
