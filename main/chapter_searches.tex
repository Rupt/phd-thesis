...
Most important for prediction is the \textbf{likelihood}
\begin{align}
L(a) &= \prob{b}{a}
\intertext{%
...
Take for example an overflow histogram bin that requires $\met > 100\,\eV[G]$,
and is part of a figure with stacked histograms from background
and (supersymmetric) signal samples.
If those histograms are normalized to unit area, they are prior distributions
for events from the background and signal hypotheses.
The likelihood function for this overflow bin is
$\prob{\met > 100\,\eV[G]}{a}$, and depends on the hypothetical origin of that
event: $a = \textrm{`background'}$ or $a = \textrm{`signal'}$.
\TODO{transformation behaviours are important}
...
When changing coordinates, the probability is a fluid of constant total volume.
...
% evidence: lower level events with different $\met$ values, bin likelihood
...
Density is a misleading term. Prior mass cannot be compressed, it can only be
moved and redistributed. It is more like depth of tea than density of air.
