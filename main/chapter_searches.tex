...
value of an estimate (over the distribution for alternative data) should equal
its known true value~\cite{pdg2020review}.
...
Blindly followed, these rules mean that the data and test statistic could be
...
This peculiarity is illustrated at the wildly popular $\alpha = 0.05$ level by
...
Inspired by the same Bayesian arguments that we used to suggest likelihood
ratios for model comparison~\cite{neymanpearson1928max}, they considered test
statistics based on likelihoods\TODO{}
...
\caption[
Tail areas of probability distributions of data from two alternative
...
probable (sic!)''
to which Lehmann appends ``(sic!)''~\cite{lehmann2011fisher}.

\TODO{consider changing our A, B to A1 A2}
\begin{displayquote}
``%
Then the two corresponding densities at the sample point are
(A1) $6.42 \times 10^{-20}$,
(A2) $5.18 \times 10^{-4}$ and the ratio of the likelihood of Hypothesis
A1 to that of A2 is $1.24 \times 10^{-16}$.
This ratio confirms the common-sense judgment that A2 is far more plausible
than A1''
...
\TODO{Confidence intervals~\cite{neyman1935Intervals, Neyman1937Outline}}
