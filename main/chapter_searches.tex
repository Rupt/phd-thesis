...
If, that is, the data are sampled under $h$ ---
what matters to the interpretation is whether an alternative hypothesis makes
the observed result \emph{more probable}.
...
on obfuscated data, so they can be interpreted in ratios.
When setting upper limits on disfavoured signal models at the LHC,
it is standard practice to use such \pvalue\ ratios name $\cls$~\cite{
read2000modified,
Read2002cls,
Murray2010heretic,
pdg2022ynf
...
.
\end{equation}
These individual \pvalues\ are sometimes labelled
$\clsb = P(s + b)$ and $\clb = P(b)$.%
\footnote{%
Despite the $\mathrm{CL}$ labels, these are not confidence levels ---
a confidence level $\gamma$ is a boundary at which \pvalues\ are selected, such
as to set limits by solving for $x$ such that
$\gamma = 1 - P(x)$~\cite{pdg1996}.
In particle physics, \pvalues\ have sometimes been labelled $\mathrm{CL}$ since
at least 1984~\cite{pdg1984}, although earlier textbooks
(cited by~\cite{pdg1984}) do make the distinction~\cite{
