...
ratio form:
\TODO{different choice for phi to avoid angle?}
\begin{equation}
...
``classical'' statistics~\cite{lehmann2011fisher, Feldman:1997qc}, but that is
inaccurate for 20th century creations that innovated on 18th century
ideas~\cite{bayes1763lii, laplace1774stigler}.
...
Horizontal lines mark regions where the likelihood exceeds $1/2$, $1/5$, or
$1/15$ of its maximum respectively.
The extents of these lines could be tabulated to usefully communicate
...
\paragraph{Fisher.}
% likelihood
Fisher coined the term `likelihood'~\cite{fisher1921probable} in the usage that
...
\end{displayquote}
and furthermore we agree that in comparison to confidence intervals,
\begin{displayquote}
\small
``%
...
reported by their likelihood's maximum and the regions in which it exceeds
...
\TODO{estimators briefly here}
\TODO{maximum likelihood}

% estimators --- not unbiased!
\TODO{
(unbiased estimators)
...
% likelihood quotes
In avoiding the pitfalls of posteriors, Fisher's use of likelihoods remains
...
$P(r=0.3)=0.142$ and
$P(r=0.95)=0.00014$.
``Thus'', Fisher concludes, ``the value $0.95$\ldots
is roughly $1000$ times less likely than $0.30$''
...
Plausibly, it is a data reduction; comparing
...
But $B: t \geq \tilde{t}$ is a logical statement (true for all test statistics
greater than or equal to the observed value) and not a randomly occurring
event, so its assignment frequentist probability is deeply dubious.
Much more importantly, the reduction to a one-sided inequality has no tangible
benefit; it reduces the information content of the reported data, but does not
reduce the costs of reporting or interpreting those data.
Both $A$ and $B$ are printed in the same number of characters, and $p$-values
can be painfully expensive to evaluate.
Furthermore, it can actively harm the result by diluting the likelihood ratio,
since if
\begin{equation}
\phi = \log \frac{L(h_1)}{L(h_2)}
\to
\log \frac{
...
\end{equation}
then large tail probabilities will only squash the $\log$ ratio toward zero.
...
(mostly $ZZ^*$) samples in Figure~\ref{fig:atlas_higgs_4l}.
If an event is observed with $m_{4\ell} = 125\,\eV[G]$, is it signal or
background?
...
In fact, this $ZZ^*$ density peaks for $ZZ$ above a
$2 m(Z) \approx 180\,\eV[G]$
edge, and $Z \to 4\ell$ around $m_{4\ell} \approx m(Z)$;
expanding the histogram to show either of these tails would change nothing
local to our $m_{4\ell} = 125\,\eV[G]$ event, but would change the $p$-values.
...
Tail areas of prior data distributions from two alternative hypotheses,
reproduced from Neyman and Pearson~\cite{neymanpearson1933lemma}.
}
...
\TODO{
maximum likelihood, p-values, confidence intervals
}
