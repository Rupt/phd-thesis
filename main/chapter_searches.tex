...
are numbers of discrete randomly occuring events, such that a frequentist
probability is a ratio of frequencies:
\begin{equation}
\label{eqn:searches_prob_frequency_ratio}
\freq{b}{a} = \lim_{N \to \infty}\frac{n}{N},
\end{equation}
where $n$ is the number observed to satisfy conditions $a$ and $b$, and
$N$ have satisfied $b$.
From Equation~\ref{eqn:searches_binomial}, this converges to the general
probability in its infinite limit.

Venn~\cite{venn1866logic},
Boole~\cite{boole1854investigation},
Kolmogorov~\cite{
kolomogoroff1933de,
...
TODO cite.
One is free to engage in such doublethink, just as one is free to proclaim that
$2 + 2 = 5$.
The fact remains that many constraints do not correspond to predictions of
auxiliary data: counterexamples include
scale variations,
theoretical cross-section uncertainties,
non-closure uncertainties (which cover differences between estimates).
simulated sample camparisons,
statistical errors on weighted simulations,
extrapolation uncertainties,
and
all uncertainties in supersymmetric signal yields
