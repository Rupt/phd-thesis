...
In the words of Skilling~\cite{skilling2008rant},
``we SHOULD use Bayes by assigning subjective priors and inspecting evidence
values.''
This framing of probabilistic data analysis agrees exactly with out standard
...
This means that we have scenario with a large fixed number $N$ of trials
(bunch collisions)
...
At a maximum $\check{x}$, the first derivative of a function vanishes and its
second derivative is negative, after Taylor expanding about its maximum this
product looks like
\begin{equation}
...
predictions.
Unlike ideal posterior updating, this does not exactly employ all information
in the data, only a function of it --- one usually cannot afford to consume all
aspects of data, so although theoretically suboptimal this approximation is
vastly superior in practice.
\item Go to 1.
\end{enumerate}
...
You are therefore incentivised to answer a) quickly, before other good guesses,
...
loss~\cite{savage1951review}) can empirically work well, this the mathematical
statement of this theorem states that restricting oneself to Bayes decision
rules cannot deny access to optimal decisions.
Alternatives can at best be equivalent.
...
\TODO{why?
not explicitly constructed cost functions for what to report...
report for contrast with attempted alternative formulations that for social...
reasons retain popularity despite contradicting decision theory...
link to frequentist ideas
Wald worked firmly in that camp - quote
}

% connection to frequentist section
Bayes' theorem is an infamous and necessary result, and it is not seriously
disputed.
What has been disputed is whether probability can be applied to logic, or any
application other than the random happenings of events.
\TODO{also ignore the results decision theory and attempt to supplant them with
