...
Although the statement in Section~\ref{sec:searches_fisher} that \pvalue\
...
\begin{equation}
\log \frac{L(s + b)}{L(b)}
= -s + n\log\!\left(1 + \frac{s}{b}\right)
\end{equation}
is linear in $n$, both $n$ and $-n$ are optimal in the Neyman-Pearson sense.
...
threshold, say $5$ events, then models can predict whether or not $n \leq 5$,
...
They know what the message says, but not that it states the whole truth.
...
and can help to better approximate a likelihood ratio ordering.

A great benefit to these modified maximum likelihood ratios is that their
sampling distributions have known approximations in asymptotic (Gaussian)
limits, which assign \pvalues\ through well-behaved analytic
...
Such simulations can be computationally expensive, and even hard to define for
``likelihood'' models that blur the line between actual data and
...
\TODO{uncertainties: systematics, statistical}
