...
simply inaccurate for a 20th century creations that innovated on 18th century
ideas~\cite{bayes1763lii, laplace1774stigler}.
Jaynes instead names them ``orthodox''~\cite{jaynes2003probability},
which may be accurate in some contexts, but alternative
...
The quintessential example is $t$-statistic
\begin{equation}
t(d) = \frac{\bar{\mu}(d) - \mu}{\bar{\sigma}(d)}\sqrt{n}
,
\end{equation}
where $\mu$ is the known data mean, and $\bar{\mu}(d)$ and $\bar{\sigma}(d)$
are standard estimates of the mean and standard deviation of the data sample
respectively~\cite{student1908, fisher1925t}.
By dividing out a (scale equivariant) standard deviation, $t$ is independent
of common scale factors on the data, and that scale invariance has helped to
drive the study and common use of this $t$ statistic~\cite{lehmann2011fisher}.

One can usefully reduce observed data to just the test statistic
$\tilde{t} = t(d)$ and compare likelihoods to compare how well different
hypotheses predict the observed $\tilde{t}$.
...
test statistic.
Equation~\ref{eqn:searches_pvalue_likelihood} emphasizes that it is just the
likelihood plus some fluff from a tail of larger test statistics.

\TODO{quote EX.II from ~\cite{fisher1921probable}
}
