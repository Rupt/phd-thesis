...
\TODO{positive message:
not meant only to disparage our lies, but to indicate that these explanations
arise to fill a hole of confusion that we can fill by reporting interpretable
...
(in fact linearly) related to the likelihood ratio.
Each $\cls$ is a ratio between signal-plus-background and background-only
\pvalues\ as defined in Equation~\ref{eqn:searches_cls}.
...
\caption{%
Ordering the data by a maximum likelihood ratio test statistic.
}
\end{subfigure}
\\[.5em]
\begin{subfigure}{\textwidth}
