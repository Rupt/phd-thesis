...
\paragraph{CR, VR, SR.}
\atlas\ SUSY searches see data only through discrete histogram bins,
...
\textbf{Control regions} (\textbf{CR}s), select large numbers data to
...
to signal hypotheses, data in \textbf{validation regions} (\textbf{VR}s) first
...
Testing is performed through in prediction of data in the
\textbf{signal regions} (\textbf{SR}s),
in which signal yields should significantly add on top of the Standard Model
backgrounds.
Some signal regions are also labelled \textbf{DR} to indicate their use in
specific `discovery' fits.

\TODO{Figure to illustrate CR VR SR extrapolation}
