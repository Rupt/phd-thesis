...
is not the mean of the squares~\cite{Barlow:2019svl}.
...
$\tilde{t}$ and an hypothesis $h$ is a \textbf{$p$-value}
\begin{align}
P(h)
&= \prob{t \geq \tilde{t}}{h}
...
;
\end{align}
the $p$-value $P(h)$ is the prior probability of data
\emph{greater or equal to} the observed data when ordered by the value of the
test statistic.
Equation~\ref{eqn:searches_pvalue_likelihood} points out that the $p$-value is
just is the likelihood plus some fluff from a tail of larger test statistics.

Fisher illustrates the use of $p$-values in the second example
...
But this factor of $1000$ is a $p$-value ratio, not a likelihood ratio,
...
shows that the $p$-value is a likelihood only if the result is reduced from
$A: t =    \tilde{t}$ to
$B: t \geq \tilde{t}$.
...
Fisher's $p$-value ratio of $1000$ is, therefore, a $\log$ likelihood
...
observed data.
This example illustrates $p$-values pathological dependency on tail selections,
and the benefits of likelihood ratios.
}
\label{fig:searches_atlas_higgs_4l}
\end{figure}

Fisher claims that the $p$-value indicates the ``significance'' of an
...
Suspecting an hypothesis when its $p$-value lands below (or above) a threshold
...
then $\alpha = 0.2$.%
\footnote{%
Exactly matching $\alpha$ is impossible for discrete distributions when
the probability masses do not add up to $\alpha$.
Neyman accommodates this by allowing inequalities with $\alpha$~\cite{
neyman1935Intervals
},
and other approaches introduce ``extraneous randomization'' to
...
Neyman and Pearson formalized this $p$-value based suspicion into decision
...
Rather than a $p$-value arousing suspicion of an hypothesis, here it is used
...
As test statistics, they also win generality by avoid context-specific
...
neymanpearson1933lemma}.
To this quotation, Lehmann adds ``(sic!)''~\cite{lehmann2011fisher}, since it
...
They authors understood the Bayesian algebra of how large likelihood ratios
make hypotheses \emph{more probable}, independent of whatever (non-zero) prior
probabilities they are assigned.
They state later in the same work that
\begin{displayquote}
``%
...
\end{displayquote}
And elsewhere in an example with two hypotheses they state that that
\begin{displayquote}
``%
...
\end{displayquote}
again relating the likelihood ratio to how ``plausible'' an hypothesis is.

The famous Neyman-Pearson lemma does not prove this usefulness of likelihood
ratios in probability algebra.
That was already known.
What it contributes is a utility for likelihood ratio test statistics
in one-sided selections --- it states that likelihood ratios
(not maximum likelihood ratios)
are optimal in a specific sense within their framework of decisions
based on small $p$-values.
Precisely, at a fixed probability $\alpha$ of rejecting a true hypothesis, a
likelihood ratio test statistic maximizes the probability
$\alpha'$ of rejecting the alternative, false hypothesis that lives in the
denominator of the ratio.
They illustrate this construction with a plot reproduced in
...
Only the likelihood ratio local to the observed data matters to the results of
a probabilistic analysis, but this lemma is relevant to the construction
of useful event variables when designing an analysis procedure.
...
like a likelihood ratio in that any right-handed selection catches a good
...
Re-ordering $m_{4\ell}$ by a likelihood ratio could turn this window into an
equivalent and possibly simpler one-sided selection.

Neyman and Pearson were also involved in the development of
`confidence intervals', which are ranges of parameters that are accepted by the
$p$-value decision rules~\cite{
clopper1934confidence,
neyman1935Intervals,
Neyman1937Outline
}.
A parameter corresponds to an hypothesis which assigns a prior distribution over
data, and those data for which an obeisant researcher accepts the hypothesis
have probabilities that sum to $1 - \alpha$.
This rule selects a set of favoured data for each hypothesis, and the
...
data `probably' contains the unknown parameter, but that is backwards.
...
For given data, the confidence interval can in fact exclude all possible
parameters~\cite{
pratt1961testing,
Jaynes1976intervals
}.
The correct interpretation of a confidence interval is explained in an example
by Pratt:
\begin{displayquote}
\small
``%
...
confusion about their intervals arises externally.

Although the confidence interval is a function of the data, so can be used
for vague inferences, the recommendation by Fisher to communicate the
likelihood function remains simpler and clearer.
Fisher deserves the last word here also:
\begin{displayquote}
\small
``%
...
$\cls$~\cite{Read:451614, Read2002cls, Murray2010heretic}
