...
have probabilities that sum to $\gamma = 1 - \alpha$.
...
.
\end{equation}
These \pvalues\ are sometimes labelled $\clsb = P(s + b)$ and $\clb = P(b)$
Some literature also defines $\clb = 1 - p_b$~\cite{
Cowan:2010js
},
where $p_b$ is not quite a \pvalue\ if it excludes equality with the test
statistic.

textbooks cited by pdg~\cite{
