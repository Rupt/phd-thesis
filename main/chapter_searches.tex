...
Probability assignments can, in general, be freely assigned and refined from
calibration data. But there are unambiguous cases that are commonly used in
High-Energy, notably the Poisson and Gaussian distributions.
...
derivations~\cite{jaynes2003probability}.

Both Poisson and
Binary accept/reject experiments are described by the
The Large Hadron Collider creates $40$~million proton collisions per second,
only around $1000$ are accepted by \atlas' trigger system per
second~\cite{atlas2020trigger}.
This acceptance rate of $250$ parts per million is sufficient to invoke the
Poisson distribution for event counts.
Each collision event has some probability $p$ of passing the trigger
\begin{equation}
\label{eqn:searches_poisson}
\mathrm{Poisson}(n\mid \lambda) = \frac{\lambda^n}{n!}\exp{(-\lambda)}
\end{equation}

Succinctly stated by Jaynes that
...
distribution of sums, but can be used to derive it: the distribution of a sum
is a convolution integral, which becomes a product in Fourier-transformed
space, and that product can converge to a Gaussian function by this rule.
Jaynes gives a beautiful alternative proof of the
Central Limit Theorem~\cite{jaynes2003probability}
(Section ``7.16 The central limit theorem'').
