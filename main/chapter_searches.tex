...
Jaynes instead names them ``orthodox''~\cite{jaynes2003probability},
...
For reasons I cannot explain, Fisher advocates a different use ---
...
then $\alpha = 0.2$ (if the distribution for is continuous or
...
On one hand they argue for the practical usefulness of maximum likelihood
ratios
\begin{equation}
\Lambda =
...
of alternatives.
As descriptions of the likelihoods, maximum likelihood ratios can usefully
communicate relevant information from the data.
...
Precisely, it states that at a fixed probability $\alpha$ of rejecting a true
hypothesis, a likelihood ratio test statistic maximizes the probability
...
Although the likelihood ratio local to the observed data is all that matters
for the probabilistic analysis, this lemma is relevant to the construction
of useful event variables for analysing our data.
Figures~\ref{fig:searches_sig_bkg_prior_likelihood}
and~\ref{fig:searches_atlas_higgs_4l} \TODO{}

\TODO{Confidence intervals~\cite{
