...
In this sense, likelihood (or evidence) ratios are scale factors by which prior
odds are updated.

The infamy of Bayes' rule stems from a fixation on the posterior distributions
from unjustified prior assignments.
Classical works by Bayes and Laplace~\cite{bayes1763lii, laplace1774stigler}
began this focus;
both assume prior densities that are uniform in their chosen coordinates, a
practice that can yield practical results in posterior inferences.
But for our purposes of reporting scientific data, such practices are fairly
criticized as arbitrary when they influce our results.
\TODO{references for definition of Bayesian analysis as computation of
posterior without even giving a symbool to the evidence}
Figure~\ref{fig:searches_baton_roue_bayes}

\TODO{zero evidence because for uniform on the real line --- a normalized prior
with any inkling of the scale of the actual parameter will be infinitely
preferred in its evidence.}
oceeds as follows:
\begin{enumerate}
