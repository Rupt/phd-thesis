...
that constrains the (possibly un-normalized or `improper') prior measure that
...
choosing a \emph{regularizer} that should be indended to induce preferable
...
we choose the letter $E$ for energy in the analogy to statistical physics where
at low temperature $T = 1/\beta$ a system collapses to its minimuum energy
state due to the Boltzmann factor
\begin{equation}
p(x) \propto e^{-\beta E(x)}
.
\end{equation}
At unit temperature, this can define a propability distribution for the
parametesr relative to an appropraite (prior)
...
\TODO{uncertainties: systematics, statistical --- maybe susy step?}
