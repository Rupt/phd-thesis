...
and $\mathrm{A}_2$, and with some data they find a $\log$ likelihood ratio of
...
in one-sided inequalities --- it states that likelihood ratios
...
denominator of the ratio; they illustrate this construction with the diagram
reproduced in Figure~\ref{fig:searches_np_tails}.
...
into an equivalent and simpler one-sided selection ---
the Neyman-Pearson lemma states that there is no better ordering for optimizing
the signal-to-background ratio available from a one-sided selection.
...
Confidence intervals are functions of data, so can inform vague inferences if
used as data for likelihood functions, as we did to interpret Fisher's ratio
$p$-value ratio as a likelihood ratio.
And again this cannot add to the data, it can only detract.
Fisher's recommendation to communicate the data and likelihood function
directly remains simpler and clearer.
