...
\exp(x) = \displaystyle \lim_{N \to \infty} (1 - \frac{x}{N})^N
.
\end{equation}
...
This is a Binomial scenario with a large fixed number $N$ of trials
(bunch collisions)
and a small probability of acceptance (triggering),
due to a notable collision event.
The Binomial distribution for $n$ triggered events is well known as
\begin{equation}
\mathrm{Binomial}(n\mid N, p) = p^n (1 - p)^{N - n} \times \binom{N}{n}
,
\end{equation}
which is a product of the probabilities to the sequence of $N$ accept/reject
data, summed over the permutations that could have led to the total $n$.
...
In the large $N$ limit, this tends towards the Poisson
...
as a practical approximation it permits analysis in linear algebra,
as the unique distribution is uniquely spherically symmetric and factors into
independent dimensions~\cite{
jaynes2003probability,
herschel1850normal,
maxwell1860normal,
muller1959note,
