...
widespread and widely reported in areas of science that still use them~\cite{
schervish1996p,
Cohen1994TheEI,
Amrhein2017TheEI,
goodman2008dirty,
...
$p_b$ might not be a \pvalue\ if it excludes equality with the observed
test statistic.
}%
...
Early forms of $\cls$ derived it from an identity with posterior tail
...
in contrast, $\cls$ (shown on the right of this figure) can only favour the
background-only model.

\TODO{also show actual likelihood ratio}
\TODO{this implements a form of propaganda that speaks only truth while
censoring all information that supports alternative ideas}
\TODO{The one-sided nature of this evidence is paradoxical, but has
resolutions.}
\TODO{If an analysis were able to `is $n \geq 5$?', it could be answered in the
affirmative or negative, and would avoid this bias.}
\TODO{Otherwise, one can fall back to not trusting the reporter ---
the statements `\atlas\ reports $n \geq 0$' and `$n \geq 0$' are not identical
since the first might suggest that in fact $n = 0$, so wise likelihoods can be
could recover the original data.
One knows what the message says, but not whether it states the whole truth.
}
\TODO{
choosing an inequality from the observed data is like designing signal
regions from the observed data
}
...
\TODO{Statistical and systematic uncertainties}
