...
associated work~\cite{close2011infinity}.%
}
which a complex doublet (equivalently four scalar fields) with an associated
potential.
Minima of the potential lie in a gauge-symmetric set, and the choice made by
a system as it settles into one particular ground state spontaneously breaks
the symmetry.
Around the chosen ground state, linear combinations of the electroweak fields
...
the reconciliation of quantum field theory with general relativity is
mostly \emph{unknown}.
...
or to derive the mathematical structures~\cite{
goyal2010origin,
skilling2021abc,
axioms1010038
}
are curious, but will remain only curiosities unless they provide novel
benefits.
...
so we use simplified models to coarsely characterize interesting selections
of plausible effects.
...
in specific simplified models, into super-$\eV[T]$ territory.
...
which could be made with data from the \atlas\ detector.
