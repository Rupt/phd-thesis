...
This simplified GMSB model also chooses to make the SUSY mass spectrum
`compressed', meaning that there are small mass differences between
$\neutralino_1$ and the slightly-heavier $\neutralino_2$ and $\chargino_2$.
...
should occur in the data with a Poisson mean of $\mu = L \sigma = 139$ times.
To achieve mean $\mu \geq 1$ event in the dataset, a process needs a cross-section
at least $1 / L \approx 7\,\mathrm{pb}$.
...
For mass scale $q$, these three factors combine to an approximate scaling
of $\sigma \sim q^{-6}$ in the several-hundred-$\,\eV[G]$ regime.
