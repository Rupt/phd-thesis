...
Angular selections on lepton variables $\rll$ and $\dphillmet$ are used to
reject $\ttbar$ and provide signal sensitivity in Low regions, which are
defined in Table~\ref{tab:2ljets_low}.
Motivations for these angular selections are displayed in
Figure~\ref{fig:2ljets_low_minus}, and discussed here.
...
Uniquely in Low regions and signal models of small mass splittings, it is
seen (from Figure~\ref{fig:2ljets_low_minus_norll_nomt2_mt2}) that some
substantial signal masses accumulate at small $\mttwoll$.
To capture these signals, SR-Low-2 is defined as the only
$\twoljets$-electroweak region with $\mttwoll < 80\,\eV[G]$.

That threshold of $80\,\eV[G]$ was chosen since after other selections for
SR-Low-2, almost all signal and background samples are in the spike near $0$;
a lower requirement would make little difference, and $80\,\eV[G]$ matches
other, usually inverted, selections.

The exclusion of $\metsig \in 9\textrm{--}12$ in SR-Low-2 is not particularly
well motivated.
