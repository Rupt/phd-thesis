...
PDFs (parton distribution functions) and phyiscal scales
...
neither being confident --- as an illustration, imagine a wide error bar and
two post-fit results right in its centre.
This effect would yield an underestimated uncertainty, so does not invalidate
the comparison approach.
On the contrary, it highlights that it is a bare minimum and that larger
uncertainties may well be reasonable given our imprecise constraints on the
Standard Model.
...
For implementations in the HistFactory model, these variations reduce to
the relative scale factors presented in Table~\ref{tab:2ljets_zjets_alt}.
...
the text.
Note how up and down variations can move in different directions, and these
different movements are coupled in the background model.
}
