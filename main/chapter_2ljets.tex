...
As motivated in the previous sections, I personal tastes for how searches
should be constructed.
One taste is for simulation-driven background estimates, since they are
interpretable as representations of the Standard Model and they can exceed the
precision available in data alone.
Another is for orthogonal regions, such that their likelihoods are independent
and the information in their data can be cleanly combined.

The core idea behind the design of the $\twoljets$-electroweak analysis is to
...
`SR2-int' and `SR2-high' regions with orthogonal, combined selections.
...
$\twoljets$ signal region from an expected sensitivity for each signal point.
Two regions and the exclusion contours from that search are displayed in
...
The rightmost bin includes overflow.
\\[0.5em]
...
The separation between SR2-low and $3\ell$ is not trivial.
}
\label{fig:2ljets_23l_stuff}
\end{figure}

Additionally, $\twoljets$ widens its scope to target a simplified GMSB model,
which has been illustrated in Figure~\ref{fig:2ljets_signal_diagrams}.
These two models behave similarly since both feature decays via massive bosons
($WZ$, $ZZ$ or $Zh$) to leptons and jets, in similarly structures decay trees
to pairs of invisible supersymmetric particles.
...
Searches by \cms\ have found comparable exclusions contours for the
C1N2 and GMSB models.
\TODO{CMS papers}
