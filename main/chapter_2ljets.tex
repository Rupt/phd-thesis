...
This topological clustering algorithm is not the latest \atlas\ jet algorithm;
it has been supplanted in recent years by a `particle flow' method, which
uses tracking information to improve the reconstruction of charged
hadrons~\cite{atlas_jet_pflow_PERF_2015_09}.
We did not switch to these improved jets because the change would have required
further delays.

Baseline jets are defined require $\pt > 20~\eV[G]$, and signal jets require
$\pt > 30~\eV[G]$.
Again, details of quality criteria and angular selections
The $\twoljets$ electroweak analysis uses only signal jets unless those jets
are $b$-tagged.

\subsubsection{$b$-tagging}
\label{sec:btagging}
Tagging adds additional labels to jets, and is particularly useful for
encoding flavour information derived from data within the jet cluster.
