...
My main messages that this thesis reports are my contributions to the recently
...
Firstly, it avoids iterating over those elements twice, once before unblinding
and again after;
search project has many little and loosely-related pieces that I attempt to
to cover locally to reduce the reader's need to jump between far-flung corners
of the chapter.
Secondly, and sadly, many design decisions can only be explained in light of
...
we add signal contributions on top of the modelled backgrounds.
...
that decay via $W\rightarrow q\bar q$ and $Z\rightarrow \ell^\pm \ell^\mp$
...
bosons, in variable branching fractions,
...
with $Z\rightarrow \ell\ell$ and $h/Z\rightarrow bb/jj$.
We explore the parameters
$m(\neutralino_1)$ and $B(\neutralino_1 \rightarrow h \tilde{G})$ with a tiny
...
cross-sections are varied up and down by theoretical error bars.
\\[0.5em]
...
as smaller signal expectations limits $S_{\mathrm{obs}}^{95}$.
...
Values with errors are rounded to one or two significant figures according
the the PDG rounding scheme~\cite{pdg2020review}.%
}
...
\item Evaluation of $Z$+jets uncertainties for RJR signal regions through
from simulations. These exceeded $100\%$ relative variation and supported the
...
Analysis quality also might be improved if our designs take lessons from
...
distribution of the invariant masses of lepton pairs that can have distinct
features that depend on the masses of decaying supersymmetric resonances.
...
supersymmetric signals from Standard Model backgrounds,
these practical issues motivate my decision to use any RJR event variables in
the $\twoljets$-electroweak analysis.
...
effects in cases where their decays include pairs of charged leptons.
...
partly because no $b$-tagging veto is applied.
...
If I had a pet theory that landed on one of these sharp
...
Since the $W$ and $Z$ boson masses are similar, this sacrifice turns out to be
quite small anyway.

Multi-bin fits are not special.
They are standard \atlas\ practice, and there should have been no complication
had I designed copied SR2-int and SR2-high for $\twoljets$-electroweak but with
with minor tweaks to change them into orthogonally binned regions.
...
that are standard elsewhere in high energy physics, and are easily analysed
by the standard likelihood fitting methods.
...
Various dubious workaround are invoked to assign plausible variations despite
small yields, which we shall in the remainder of this chapter.
...
Since we designed these searches use orthogonal datasets, they may be
...
$2/3\ell$ compressed~\cite{atlas_susy_compressed_2l_2018_run2},
...
Exclusion of the largest $B(\neutralino_1 \rightarrow h \tilde{G})$ values
requires sensitivity to double-Higgs decay signatures --- \cms\ have published
exclusions in the region combining $hh\rightarrow \gamma\gamma+\met$ and
$hh\rightarrow b\bar bb\bar b$ with other channels to exclude
$B(\neutralino_1 \rightarrow h \tilde{G}) = 1$ up to
...
two charged leptons (those are electrons or muons --- not taus),
and at least one jet.
Though not in the name, a $\met$ requirement is also implicit from the context
...
Our dataset comprises $136.0\,\mathrm{fb}^{-1}$ ($\pm 1.7\%$) of integrated
...
There would be no event variables if not for the trigger reducing the data to
a manageable size.
When living our lives as humans, we are bombarded with sensory data and must
pay attention only those gems few of any use, to avoid exhausting our
processing capacity.
...
We use use only the `lowest' (loosest, highest rate) triggers selecting two
...
To avoid these thresholds, all leptons selected for the $\twoljets$-electroweak
regions are required to have $\pt > 25\,\eV[G]$.
...
Therefore, any additional trigger has at most a $10\%$;
...
The $\twoljets$ team considered that to be too little to be worthwhile.
...
this is because taus decay before reconstruction so are used only indirectly
through their decay products, and neutral leptons
--- neutrinos --- go missing and contribute to the $\met$.
...
kinematic and is identified with `quality' criteria.
...
lepton counts, such that our results might be combined in future to deliver an
...
Unlike how leptons are tagged as electrons and muons with very clean
separation, jet tagging is a rather more noisy affair given \atlas' limited
...
reject $\ttbar$ events and select for $h\rightarrow b\bar b$ decays.
...
considered too much additional work to considered in $\twoljets$,
...
This algorithm is trained on a mixture of simulated $\ttbar$ and $\zjets$ data,
and the reported efficiency means that a true $b$-jet sampled from the
algorithm's testing dataset has an estimated $77\%$ probability of being
tagged.
The `c10' suffix means that those data include a $7\%$ fraction of
...
Once we know about all of the reconstructed leptons and hadronic jets, we can
also improve our rejection of non-prompt leptons from hadron decays, since
leptons near to jets become suspect.
...
Please note that $\met$ is not an energy; transverse energy does not exist.
...
for example, in a $Z\mathrm{+jet}$ event $pp \rightarrow Zg$, with the $Z$ and
...
Signal models in $\twoljets$ generate hard, invisible particles.
By selecting for large $\met$, however, we select events in which either there
were real invisible particles or something was badly mismeasured.
And the rate of bad mismeasurements from Standard Model processes
with large cross-sections can be substantial.
...
Unless, as discussed in Chapter~\ref{chapter:experiment}, that muon is hard
...
objects in an event, this $\Sigma$ is a sum of covariances assigned to those
objects; both means and variances add linearly.
...
resonances, at the LHCs, $\mttwo$ remains useful for separating standard model
...
denotes the four-vector with energy $E$ and momentum
...
outside of their selected mass windows.
...
all of these event variables are estimates derived from approximate
reconstructions.
...
requirement of the basic $Z\rightarrow \ell^+\ell^-$ plus jets plus $\met$
...
and the the whole $\twoljets$ team for collaboration in their finalization.
}
...
The highest $\metsig$ selections, starting around $\metsig > 18$, contain
...
both also produce resonant $Z\rightarrow \ell^+\ell^-$ pairs
...
captures both $W\rightarrow jj$ and $Z\rightarrow jj$ for C1N2 and GMSB models
...
capture $Z\rightarrow bb$ decays, which become significant for signal models
...
$W\rightarrow jj$ distributions, but a $\ptjone$ selection is found to be
...
These Low regions target signal models with small mass splittings,
of around $100\textrm{--}150\,\eV[G]$ that still exceed the masses of
electroweak bosons.
In particular, we aim to test C1N2($200$, $100$), which is a light, on-shell
benchmark that has not previously been excluded in this
channel~\cite{atlas_23l_SUSY_2016_24}.
...
$\metsig \in 6\textrm{--}9\textrm{--}12$ to form SR-Low-a and SR-Low-b
respectively.
...
requirement, and can be plausibly understood from scenarios where a the boosted
supersymmetric resonance decays to $Z+\mathrm{invisible}$ and sends both
the $Z\rightarrow \ell^+\ell^-$ and invisible particle, which becomes
...
and such a cut may well have improved the analysis in this case.
...
retreating to the SR-Low requirement of exactly jets.
...
Early OffShell designs used the `ISR' construction that was described
...
needing reconstructed jets for a $W\rightarrow jj$ candidate.
...
An alternative ``shapefactor'' prescription exists which gives one parameter
per sample per bin, but this greatly increases the complexity of the model and
...
estimating a linear relationship between dilepton event rates that is
...
Matrix Element Method~\cite{gainer2013matrix} for signal discrimination.%
}
...
respectively.
These transition probabilities $p(*\!\mid\!*,i)$ are estimated by studies in
...
estimates, then flops those back to data-level estimates. Flip\ldots flop.
...
in most regions.
The exceptions to this rule are all regions the High category,
...
that determine jets reconstructed masses.
...
capture their targets, and he focus from \atlas' performance operations is
...
necessarily mean up and down variations of bin yields, but to the coupled
variations in directions given by the signs of the relative variations given in
the various regions.
...
Internal reviewers were, there, confused as to why we mentioned a deficit in
...
we do not expect large $W\rightarrow jj$ contributions in this \diboson\
...
Scale variations now appeared twice for signals:
...
This section presents some highlights these validation studies.
...
$ZZ$, with one $Z\rightarrow \ell\ell$ and the other invisibly to
...
Perhaps what should be the most meaningful result is the observed upper limit
$S_{\mathrm{obs}}^{95}$.
The analysis of each region sets its upper limit by scanning different values
...
believed to work.
For consistency, we therefore use the alternative toy simulation
approximation for all discovery regions.
...
The exact pruned models are shared in our \hepdata\
archive~\cite{hepdata.116034}.
Not only do these simulations take computational time, they also suffer
...
calculate statistics that are so deeply flawed causes me some shame.
In a vain attempt to absolve myself of this sin, and to positively encourage
honest reporting of the data, I include with it the following note:
\emph{%
...
--- as a probability that data occurred at random, by chance, or by statistical
fluctuation,
...
processing on around $1500$ processor cores, accumulating to around a million
core hours.
...
Many test statistic distributions are involved here --- two (background-only
and signal-plus-background) at each scanned point, and one additional
distribution per region (with yet another test statistic) for the discovery
$p$-value $p_{s\!=\!0}$.
From these, a few examples are presented in
Figure~\ref{fig:2ljets_test_stat_distributions}
to investigate curious features of the results.
...
By taking a ratio of $p$-values, $\cls$ approximates that likelihood ratio:
...
corresponds to pseudo-data with $n=0$; the peaks to its left correspond
...
(adding signal just adds a constant to the $-\log$ likelihood objectives being
minimized), but its distribution is not quite a delta function since the test
statistic depends on the noisy termination of numerical optimization
algorithms.
Resulting $\clb$ and $\clsb$ values depend on this effect ---
...
This small $\clb$ value could appear to be an anomalously large significance
if that $p$-value were  misinterpreted as a measure of compatibility with the
background model.
Thankfully, it is remedied by the $\cls$-prescribed ratio with $\clsb$,
since both $p$-values suffer from similar effects.
Statistical precision is harmed, however --- although $1$m samples should
...
of~\cite{SUSY-2018-36}, which reports a background expectation of
$1.08 \pm 0.18$ and expected limits of
$S_{\mathrm{exp}}^{95} = 3.8^{+1.3}_{-0.5}$, but zero observed data with a
whopping $S_{\mathrm{obs}}^{95} = 4.8$.
Using their \pyhf\ models published through \hepdata, I have failed to
reproduce this effect outside the \histfitter-\roofit\ system, finding instead
a limit close to $3$. It would not surprise me if similar complications with
the test statistic distribution played a part.
...
exceeds the central expected limit.
Another public example of this effect is
$\mathrm{SR}0\textrm{-}\mathrm{ZZ^{tight}_{bveto}}$ of~\cite{SUSY-2018-02}.
\emph{Thanks to Tina~Potter discussions of this problem and for sharing
its details of its modelling.}
...
distribution.
Fundamentally, a sufficient explanation of this problem is our refusal to
compare models with likelihood ratios.
...
the Neyman–Pearson lemma~\cite{neyman1933ix}.
...
Limits moving in the wrong directions can simply be explained by flawed
test statistics ranking data in a bad order.

A second sufficient explanation for these paradoxical limit results is also
that the toy sampling method does not assign legitimate $p$-values because its
test statistic distributions are assigned by first fitting to the
data~\cite{CMS-NOTE-2011-005}.
...
flexible signal model, and should therefore, to my understanding, be always
non-negative.
...
Histogram ranges in Figure~\ref{fig:2ljets_test_stat_distributions} are
set from the smallest and largest toy test statistics, and their large extent
indicates rare outliers that may also result from failed fits.

It is certainly arguable that the pathological test statistic distributions
...
define in silicon the standard statistical methods for \atlas\ SUSY searches.
To diverge from them would make our result incomparable.
...
can be like working with quick-set cement. You mold your ideas into a certain
shape that can be played with, and in the process of playing with them they
begin to harden and set until they're immutable like rock, and at that point
you can't change the world --- not without breaking it into pieces and starting
...
From \atlas' large and valuable Run~2 dataset, our $\twoljets$ searches
...
research programme in this $2\ell$+jets+$\met$ channel.
My focus has been on the $\twoljets$-electroweak search within this project.
Its result is a central pillar of the visible effects of both C1N2 and GMSB
simplified supersymmetric models, and uniquely contributes exclusion
(under the $95\%$ $\cls$ prescription) of certain regions of the models'
parameter spaces.
...
signal regions, their overly ambitious design in the $\twoljets$-electroweak
did come with costs:
% mm fakes
...
and struggles to reconcile their standard prescriptions for coupled
(correlated) variations with physical intuition between our many bins,
% computational
and computational costs particularly in the `toy' simulations that assign our
statistical results (Section~\ref{sec:2ljets_results}).
The root of this problem that in diverging from standard designs we
...
Although these regions are sensitive, as demonstrated by the extent of their
projected and observed exclusion contours, delays are themselves a meaningful
cost.
...
Simplicity in design may therefore have afforded a higher quality result by
reducing these delays and publishing its result years sooner,
and so implemented \cern's goal of
``accelerating science''~\cite{Radford:1551933}.

% this extra line activates enlarged spacing for the final paragraph lol
