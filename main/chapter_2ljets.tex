...
displays histograms in a `CR-Z-met', which is modifies CR-Z
...
region.
To avoid the work of evaluating variations in CR-Z-met (and all of its bins),
its variations are linearly extrapolated from CR-Z.

Good modelling in CR-Z-met is a positive sign for our simulations prior to
their modification in the fit.
Noting that CR-Z excludes $m(W)$, however, this does not conflict with the idea
...
$WZ$-like process has almost equal yields.
All of those $WZ$ events therefore have a missing third lepton, that could be
...
the effect is enhanced by the fact that $WZ$ has a larger cross-section than
$ZZ$ by around a factor of three~\cite{Campbell:2011bn}, that is enhanced
by the sum over $W^+Z$ and $W^-Z$ processes.
...
Figure~\ref{fig:2ljets_splits_zjets} shows CR-Z with its $\zjets$ yields
