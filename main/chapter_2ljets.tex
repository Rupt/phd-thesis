...
\emph{Thanks to Jon~Long for collabouration in preparing this HEPData archive,
...
It is not a magician's card trick, but an honest (if flawed) attempt to make
useful estimates.
...
with \madgraph\ (MG5\_aMC@NLO~2.2.3~\cite{Alwall:2014hca}) at leading order with
...
the ATLAS SUSY group~\cite{atlas_twiki_susy_theory_comparison} that says to
\begin{itemize}
...
estimates~\cite{atlas_twiki_pmg_theory_comparison} in for two-sample
comparisons.
...
More senior members of the \atlas\ SUSY group decided that we should keep it as
described, reasoning along the lines that
...
Much to my ire, all mentions of an SR-Low deficit were therefore removed from
the paper.
Despite bring real and significant, this deficit was erased from public view,
and to my knowledge is only reported through the inner details of
the HistFactory models shared in our HEPData~\cite{hepdata.116034}.
...
its diboson yield peaks in the signal region on $\mjj \approx m(W)$, but
we do not expect $W\rightarrow jj$ in this process since one boson is needed
for the leptons and the other for the $\met$ that we select.
...
To my knowledge, this is simply a limitation from what \atlas\ has decided to
spend its resources on, and quite reasonably since full simulation is
computationally expensive.
We did, however, have access to the cheaper alternative of set of small,
...
\emph{I am grateful to Jon~Long for preparing our initial
...
In these looser regions VR-sys-main and VR-sys-OffShell, each variation
reduces to around $10\%$ and remains dominated by the statistical uncertainty.
Although local effects could occur, this a reasonable indication that these
...
had had some meaningful variations to apply in those regions, but I have no
way of knowing how large those variations would be.


\subsubsection{Alternative \texorpdfstring{$\ttbar$}{ttbar}}
\label{sec:2ljets_ttbar_alt}
\TODO{}
