...
The benefit of an additional trigger could be increased signal yields, if that
trigger do not also harmfully increase inseparable backgrounds.
A study, \textit{which was performed by Knut Vadla}, compared the efficiencies
...
Therefore any additional trigger can therefore have at most a $\approx10\%$;
improvement, and adding one-lepton triggers was found to increase efficiency by
only a few percentage points, depending on the model.
We considered that to be too little.

Costs of triggers are attributable to human effort; adding triggers requires
software work to include them and their associated Monte Carlo weights, along
with additional studies to validation of background estimates and evaluation of
...
Although jet and $\met$ triggers were not studied in detail, they require
hard objects, such as $\ptjone \gtrsim 400~\eV[G]$ or
$\met \gtrsim 100~\eV[G]$~\cite{atlas_twiki_lowest_unprescaled}.
Leptons are striking signals which \atlas\ can trigger on with much softer
requirements; lepton triggers allow us to use softer jets and $\met$,
which are relied upon throughout the $\twoljets$ analyses.
