...
random seeds, with software that I name
...
as it bails the leaks overboard.
...
This would appear to be an anomalously large significance if $\clb$ were
misinterpreted as a measure of compatibility with the background model.
...
And with $\sqrt{n}$ precision, that easily explains the difference between
limits at $3.0$ and $2.93$ despite the huge sample size.

\TODO{}
% real world costs to pretending that our uncertainties are auxiliary data
% not only idle Bayesian-versus-Frequentist philosophy
% other examples - cite 4.6 limit.
...
this would analytically be a delta spike, but it acquires width from the finite
precision of numerical optimization algorithms.
