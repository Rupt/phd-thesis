...
\section{Events and their features}
...
so as to avoid the need to model their jagged activations.
...
for example, in a single-top event $pp \rightarrow tq$, the $t$ and $q$ are
produced back-to-back with equal momenta $p$, but the two particles have very
different energies since they have very different masses:
$E_t^2 = m_t^2 + p^2$, $E_q^2 = m_q^2 + p^2$, and $m_t \gg m_q$.
% hobbyhorse
...
a typical $\met$ noise scale of $\approx 10--30\,\eV[G]$, which I observe
by inspecting $\met / \metsig$ in $\twoljets$-electroweak selections;
$\metsig$ is defined in the next section.
