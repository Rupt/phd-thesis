...
and considering what would happen of those event yields were added to a
...
Figure~\ref{fig:2ljets_contours_c1n2} for C1N2 and
in~\ref{fig:2ljets_contours_gmsb} for GMSB.
...
Values with errors are rounded according the PDG rounding
scheme~\cite{pdg2020review}.%
}
...
Various workaround are invoked to assign plausible variations despite
small yields and failing statistical precision, which shall be seen in the
...
for the $\twoljets$ analyses, and which event variables are extracted for use
in its selections.
...
thresholds and the variable efficiencies near to those thresholds~\cite{
...
We in the $\twoljets$ team considered that to be too little to be worthwhile.
...
Unlike lepton flavor tagging, which can very cleanly separate electrons from
