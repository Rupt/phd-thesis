...
To avoid this extreme effect, I searched for alternative configurations
to the background model, such as constraining the parameter to $1$-sigma.
More senior members of the \atlas\ SUSY group decided that we should keep it as
described, reasoning along the lines that
a) this is a real and significant deficit in SR-Low that must appear as either
a pull on systematic variations or as a deviation from data, and
b) we prefer the pull on systematic variations.
With this decision, post-fit results showed no deficit;
this $\zjets$ sample comparison is the main reason why
Figure~\ref{fig:2ljets_summary} shows no significant in SR-Low-a or SR-Low-b.
Internal reviewers were then confused as to why we mentioned a deficit in
SR-Low, so, much to my ire, all mentions of an SR-Low deficit were erased from
the paper.
