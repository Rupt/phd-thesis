...
The different-flavour versions are contain increased $\ttbar$ backgrounds,
...
a additional validations were also inspected with different-flavour or
...
same-charge leptons, and adds a selections of $\mll \in (12, 111)~\eV[G]$
...
identify and remove remove FNP contributions from simulations.
...
Despite any awkwardness, the its does appear that FNP backgrounds are indeed
...
Figure~\ref{fig:2ljets_jetm_c1n2_600_0} for a benchmark C1N2 model in
the fast simulation, showing that most jets have small masses but that
a substantial fraction appear in a bump around $m(W)$.
To check that a similar bump exists in data and background simulations,
I conducted a study into single-lepton events to search for $\ttbar$ events
in which one top decays leptonically, and the other to a boosted hadronic $W$
boson that is reconstructed into a single small-radius jet.
Since our main samples require two leptons, this study required independent
inputs derived from different sources, so to reduce the required computing time
I opted to include only $\ttbar$ and single-top backgrounds.
To verify the origins of high-mass jets, partonic truth information is used
to label those events containing a $W$ boson decaying to two quarks that are
within $\Delta R < 0.4$ of the hardest reconstructed jet.
The resulting bump is visible in Figure~\ref{fig:2ljets_jetm_ttbar_mw} in a
selection described in its figure caption, and indeed contains the targetted
boosted $W$ boson. This positive result was an early motivation for our
continued pursuit of the 1J regions.
...
$m(\neutralino_1) = 0$.
All jets in all events in this file are histogrammed without weights,
...
\begin{figure}[tp]
\centering
\includegraphics[width=0.9\textwidth]{figures/2Ljets_hist1d_jetM0_ttbar_merged_tweak.pdf}
\caption[
...
Our inclusion of SR-High-1J and VR-High-1J regions induces two related problems.
First is their unusual use of the jet mass variable $\mjetone$ with
small-radius ($R=0.4$), topologically clustered jets~\cite{atlas_jet_topo_PERF_2014_07}.
Usually, hadronic decays of massive objects
(such as $W$, $Z$, or $t$ resonances) are searched for with large-radius jets
with $R=0.8$, since these require less extreme boosts to capture their targets.
The focus from ATLAS' performance operations is therefore reasonably to
understand these common use-cases first.
Second is the use of fast simulation~\cite{SOFT-2010-01} for our signal models,
which uses approximate descriptions of the detector response and does not
guarantee to model the substructure that determines a jet's mass.

Official jet mass scale uncertainties for small-radius jets in the full ATLAS
simulation were released late into the development of this search
(25 April 2021), and these uncertainties explicitly do not support the
fast simulation~\cite{atlas_twiki_smallrjms}.
Once released, these uncertainties were evaluated for the diboson and $\ttbar$
contributions, which are the primary backgrounds in both 1J regions.

% TODO
% alternative for fast simulation

% also apply this method to minor backgrounds
