...
The the correlation matrix of parameters, estimated from an approximate Hessian
...
\footnote{%
Pruning is, however, the reason why the signal patches (differences from
background-only to signal) in our serialized \hepdata\ models are so large;
they implement many small changes due to different decisions made by the
pruning implementation.
}

We further reduce computational costs by sing the asymptotic approximations
...
$\cls = 0.05$.
\emph{%
Thanks to Larry~Lee for sharing contour preparation scripts that were adapted
...
Figure~\ref{fig:2ljets_contours_gmsb}, but I decided it was preferable to keep
the simplicity of having the same interpolation settings for both contours.

Initially, the contours showed more glaring wobbles.
These were removed by repeating the fits of anomalous signal points until they
behaved.
...
We additionally display the $\cls$ values, from which observed contour derives,
in Figure~\ref{fig:2ljets_fit_contours_cls}.

\begin{figure}[tp]
\centering
\begin{subfigure}{0.48\textwidth}
\centering
\includegraphics[width=\textwidth]{figures/2ljets_c1n2_Output_Contours_cls_obs.png}
\caption{C1N2}
\end{subfigure}
\hfill
\begin{subfigure}{0.48\textwidth}
\centering
\includegraphics[width=\textwidth]{figures/2ljets_gmsb_Output_Contours_cls_obs.png}
\caption{GMSB}
\end{subfigure}
\caption[
...
\TODO{consider moving this to region design?}
