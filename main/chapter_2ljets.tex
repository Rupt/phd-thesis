...
honest and honestly uncertain estimates for these fake/non-prompt backgrounds.

Although my style of spelling out fake/non-prompt every time
...
\end{align}
To make fake/non-prompt estimates in our regions, the Matrix Method is
implemented by assigning weights to observed data events that are then
histogrammed them in the tighter selections of our search
regions much like other samples~\cite{twoljets2018int}.
...
are the same data in which we are searching for anomalies.
...
data in a diluted way, such that they can end up not making much difference.
...
Loose data do have positive weights and they would drive up the fake/non-prompt
background.
...
\subsubsection{Fake/non-prompt estimates in $\twoljets$-electroweak regions}
Many histograms were inspected to validate the Matrix Method estimates in
$\twoljets$-electroweak regions.
To give a broad summary across all of these regions, we define an additional
validation region, named VR-SS, to inspect similar data with same-charged
(same sign, SS) leptons.
This selection enhances fake/non-prompt contributions relative backgrounds
since it suppresses contributions $Z\rightarrow \ell^\pm\ell^\mp$ processes,
whose leptons should be oppositely charged if accurately reconstructed,
but does not suppress fake/non-prompt backgrounds because their lepton charges
are expected to be less dependent:
fake/non-prompt backgrounds produce $\ell^\mp\ell^\mp$ and $\ell^\pm\ell^\pm$
vents at more similar rates.

For these reasons, VR-SS is defined as a modification to the pre-selection
requirements of Table~\ref{tab:2ljets_presel}, in which the requirement
of opposite charge leptons is changed to a requirement of same charge leptons
(same sign, SS). Additionally, VR-SS requires $\mll \in (12, 111)~\eV[G]$,
...
with fake/non-prompt estimates taken from the Matrix Method in
...
identify and remove remove fake/non-prompt contributions from simulations.
...
two orange histograms is the simulated fake/non-prompt estimate.
...
Drafts of the $\twoljets$ paper included
Figure~\ref{fig:2ljets_vrss_mm} and a similar plot for the
$\twoljets$-strong search, and repeated this point that the deviation was not
very substantial.
These plots were removed at the request of internal reviewers, who argued that
they did not need to be shown since the fake/non-prompt background is not
very important.
To demonstrate that it was not important, we were first instructed to repeat
the exclusion fits of the $\twoljets$-electroweak analysis with an additional
uncertainty, to further cover the deviation observed in this figure, which
scaled the Matrix Method yields by a $\pm 50\%$ scale factor.
The utility of this is somewhat questionable since that variation is coupled
between all bins, so it could not vary to explain an excess in one region
without also increasing the fitted Matrix Method backgrounds in all other
regions, and it would struggle to make any significant difference.
...
largest being $2\%$ reduction in the limit on
...
fake/non-prompt leptons under the heading `MC Prompt' and the, and only
the simulated fake/non-prompt contributions under the heading `MC FNP'.
...
fake/non-prompt background estimate, and consider a maximum across them for
each region.
...
\end{itemize}
All low variations are set to zero.
...
An awkward fact in the history of this fake/non-prompt estimate is that it
...
estimates left more regions with non-positive estimate than previously.
To assign them without being biased by the unblinded signal region data,
known to contain no significant excesses, therefore took some barefaced
honesty.
...
\footnote{%
Of course $p$-values \emph{do} depend on what might have happened with
alternative data
...
Background yields from prompt (two real, prompt leptons) and fake/non-prompt
