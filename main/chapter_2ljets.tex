...
% con
Unfortunately, these requirements limit only when data-driven estimation
procedures, are defined, and do not limit the content of those procedures.
Indeed, the rules explicitly allow methods like the Matrix Method
...
No sensible research would employ an estimation procedure that clearly set
a prediction directly from the data it is supposed to predict, but our use of
the Matrix Method is proof that sufficiently obfuscated violations are allowed.
...
If blinding is to be taken seriously, then restrictions must be placed on such
pre-selection plots.
...
Despite my concerns expressed in this section, our post-unblinding changes are
relatively minor.
...
Pre-fit yields, which can plausibly be interpreted as predictions, and other
additional results are presented in the next section.
