...
Any sensitivity to these signals requires them to be visible above the
background noise.
For our purposes, that background noise arises from known processes of the
Standard Model.
After selecting parts of data that are sensitive to our C1N2 and GMSB signals,
it turns out that the most important backgrounds are describable as
$ZZ$-like \diboson,
$\zyjets$,
or $\ttbar$;
these processes are illustrated in Figure~\ref{fig:2ljets_feynman}.
Each of these Standard Model processes produces visible lepton pairs, jets, and
missing transverse momentum to mimic our signal models' effects.
Although $\zyjets$ events lack prompt invisible particles for missing
transverse momentum, this is offset by its large production cross-section
with missing transverse momentum sourced from indirect effects such as
mismeasured jets.
...
\centering
\includegraphics[width=\textwidth]{figures/2ljets_feynman_diboson_zz.png}
\caption{$ZZ$-like \diboson}
\end{subfigure}
\hfill
\begin{subfigure}{0.46\textwidth}
\centering
\includegraphics[width=\textwidth]{figures/2ljets_feynman_zyjets_slash.png}
\caption{$\zyjets$}
\end{subfigure}
\\
\begin{subfigure}{0.4\textwidth}
\centering
\includegraphics[width=\textwidth]{figures/2ljets_feynman_ttbar.png}
\caption{$\ttbar$}
\end{subfigure}
\hfill
\begin{subfigure}{0.46\textwidth}
\centering
\includegraphics[width=\textwidth]{figures/2ljets_feynman_top_decay.png}
\caption{Top quark decay}
\end{subfigure}
\caption[
Example Feynman diagrams for the Standard Model background processes
]{%
Example Feynman diagrams for the Standard Model background processes of
important background contributions in the $\twoljets$-electroweak search.
In \diboson\ events we expect one $Z$ boson-like decay to charged leptons, and
the other to neutrinos; additional jets can arise from initial state radiation
or additional strong radiations from the hard scatter.
Missing transverse momentum is absent from prompt emissions in $\zyjets$
events, as illustrated in (b), but arises from rare mismeasurements or
non-prompt decays.
Top quarks rapidly decay weakly, mainly to $b$-flavoured hadrons due to the
small weak mixing with the third generation;
panel (d) illustrates this with a leptonic decay that we expect to select.
}
\label{fig:2ljets_feynman}
\end{figure}
...
$S_{\mathrm{obs}}^{95}$ in this table.
\footnotetext{%
...
used in $\twoljets$.
A more detailed listing of our trigger choices is presented in
Table~\ref{tab:2ljets_trigger_int}.
...
\centering
\begin{tabular}{ccll}
Year & $\ell\ell$ & Trigger & Comment
\\[0.2em]
\hline
2015 & $ee$     & {\footnotesize HLT\_2e12\_lhloose\_L12EM10VH} & \\
2015 & $\mu\mu$ & {\footnotesize HLT\_2mu10} & \\
2015 & $\mu\mu$ & {\footnotesize HLT\_mu18\_mu8noL1} & \\
2015 & $e\mu$   & {\footnotesize HLT\_e17\_lhloose\_mu14} & \\
2015 & $e\mu$   & {\footnotesize HLT\_e7\_lhmedium\_mu24} & \\[0.8em]
%
2016 & $ee$     & {\footnotesize HLT\_2e15\_lhvloose\_nod0\_L12EM13VH} & except run 300540 \\
2016 & $ee$     & {\footnotesize HLT\_2e17\_lhvloose\_nod0} & run 300540, periods D4-- \\
2016 & $\mu\mu$ & {\footnotesize HLT\_2mu10} & period A \\
2016 & $\mu\mu$ & {\footnotesize HLT\_2mu14} & from period B \\
2016 & $\mu\mu$ & {\footnotesize HLT\_mu20\_mu8noL1} & periods A-D3 \\
2016 & $\mu\mu$ & {\footnotesize HLT\_mu22\_mu8noL1} & periods from D4 \\
2016 & $e\mu$   & {\footnotesize HLT\_e17\_lhloose\_nod0\_mu14} & \\
2016 & $e\mu$   & {\footnotesize HLT\_e24\_lhmedium\_nod0\_L1EM20VHI\_mu8noL1}\hspace{-3em} & \\
2016 & $e\mu$   & {\footnotesize HLT\_e7\_lhmedium\_nod0\_mu24} & \\[0.8em]
%
2017 & $ee$     & {\footnotesize HLT\_2e17\_lhvloose\_nod0\_L12EM15VHI} & except periods B5--B8 \\
2017 & $ee$     & {\footnotesize HLT\_2e24\_lhvloose\_nod0} & \\
2017 & $\mu\mu$ & {\footnotesize HLT\_2mu14} & \\
2017 & $\mu\mu$ & {\footnotesize HLT\_mu22\_mu8noL1} & \\
2017 & $e\mu$   & {\footnotesize HLT\_e17\_lhloose\_nod0\_mu14} & \\
2017 & $e\mu$   & {\footnotesize HLT\_e26\_lhmedium\_nod0\_mu8noL1} & \\
2017 & $e\mu$   & {\footnotesize HLT\_e7\_lhmedium\_nod0\_mu24} & \\[0.8em]
%
2018 & $ee$     & {\footnotesize HLT\_2e17\_lhvloose\_nod0\_L12EM15VHI} & \\
2018 & $ee$     & {\footnotesize HLT\_2e24\_lhvloose\_nod0} & \\
2018 & $\mu\mu$ & {\footnotesize HLT\_2mu14} & \\
2018 & $\mu\mu$ & {\footnotesize HLT\_mu22\_mu8noL1} & \\
2018 & $e\mu$   & {\footnotesize HLT\_e17\_lhloose\_nod0\_mu14} & \\
2018 & $e\mu$   & {\footnotesize HLT\_e26\_lhmedium\_nod0\_mu8noL1} & \\
2018 & $e\mu$   & {\footnotesize HLT\_e7\_lhmedium\_nod0\_mu24} & \\
\end{tabular}
\caption[
Detailed trigger specifications
]{%
Detailed trigger specifications for the $\twoljets$ analysis in all four
years of Run~2.
These are the dilepton triggers with the weakest available requirements.
Some information can be scraped from the name of each trigger.
Run numbers and periods mentioned in the ``Comment'' column indicate specific
parts of each LHC run, and are chosen on the advice of internal \atlas\
documentation and relate to local operating
conditions~\cite{atlas_twiki_lowest_unprescaled}.
This table is adapted from supporting material for the $\twoljets$ search
paper~\cite{atlas2022searches}.
}
\label{tab:2ljets_trigger_int}
\end{table}
...
The power of each of these variables to select signals and exclude backgrounds
is hugely variable across our many signal models and regions of phase space.
For illustration, Table~\ref{tab:2ljets_ewk_acceff} reproduces some signal
efficiency numbers for the sum of yields in all $\twoljets$-electroweak signal
regions.
It finds a that only $0.0003\%$ of signal events are captured under the light
C1N2 model with
$m(\chargino_1, \neutralino_2) = 100\,\eV[G]$ and
$m(\neutralino_1) = 60\,\eV[G]$.
The reduced cross-sections for models with large masses are offset by their
more spectacular appearances, with a much larger capture rate of
for more extreme models.
For example, the corresponding capture rate is $0.64\%$ for the C1N2 model with
$m(\chargino_1, \neutralino_2) = 650\,\eV[G]$ and
$m(\neutralino_1) = 250\,\eV[G]$.
Detailed cut-flow tables,
Tables~\ref{aux:ewk:cutflowSRllbb} to~\ref{aux:ewk:cutflowSROffShell_2} of
Appendix~\ref{app:2ljets_cut_flows},
give more granular information into the effects of many intermediate
selection requirements across all signal regions for several representative
signal models.
Notably, $\mttwoll$ rejects substantial amounts of signal, although this
rejection is worthwhile for its larger rate of background rejection.

On background rejection performance,
Tables~\ref{tab:cutflows_CR-VZ} to~\ref{tab:cutflows_CR-DY} of
Appendix~\ref{app:2ljets_cut_flows} present cut-flow tables in the four control
regions of $\twoljets$-electroweak, with background sample rates included.
These tables show that $\mttwoll$ selections are powerful for removing $\ttbar$
backgrounds, as expected, and that $\metsig$ selections beyond the
pre-selection requirement of $\metsig > 6$ drastically reduce most other
backgrounds.
Summed across all four control regions with data given in these tables,
the control region selections reduce background rates with a survival fraction
of $1.5\%$ relative to the pre-selection requirements of
...
\centering
\begin{tabular}{ccl}
\multicolumn{3}{c}{C1N2}   \\
$m(\chargino_1, \neutralino_2)$ ($\eV[G]$)
& $m(\neutralino_1)$ ($\eV[G]$)
& $A\times\epsilon$ (\%)
\\[1.5ex]
\hline
% 0.0000028408852207408747
100 & 60 & 0.0003       \\
% 0.000007322904893685767
150 & 70 & 0.0007       \\
% 0.00041538437098791273
250 & 100 & 0.042       \\
% 0.002093724286276909
400 & 200 & 0.21     \\
% 0.0063561141231079175
650 & 250 & 0.64     \\
\end{tabular}
\\[2em]
\begin{tabular}{ccl}
...
& $A\times\epsilon$ (\%)  \\[1.5ex]
\hline
% 0.00023984225213678648
200 & 50 & 0.02  \\
% 0.002095514599015893
500 & 80 & 0.21   \\
% 0.0161139463793303
700 & 0  & 1.6   \\
\end{tabular}
...
Acceptance times efficiency (fraction of total production yields) in the union
of all $\twoljets$-electroweak signal regions, displayed for example C1N2 and
GMSB signal models.
Acceptance $A$ and efficiency $\epsilon$ are defined via truth-level
simulations as specified in the footnote from Table~\ref{tab:2ljets_discovery}.
This table is adapted from the $\twoljets$ paper~\cite{atlas2022searches}.%
}
\label{tab:2ljets_ewk_acceff}
\end{table}
...
When designing the regions described in this section, I conducted a manual
search and optimization with human feedback from the $\twoljets$ team.
Expected significance statistics were used to assess the sensitivity of a
region design to particular representative signals, but final decisions were
made to maximise the extend of projected exclusion contours as evaluated by our
...
inspected the data in SR-Low-ISR (this signal region was never unblinded),
I include them here because they reveal some interesting context to the
surviving analysis design.
...
including signal regions, early~\cite{ATLAS:2022swp}.
...
and the mass splitting $m_a - m_b$, and we want an extrapolation scheme that
...
Figure~\ref{fig:2ljets_summary}.
Unlike Figure~\ref{fig:2ljets_summary}, this figure does not adjust its
...
collider (FCC-hh) might be preceeded by an electron-positron collider (FCC-ee)
with centre-of-mass energies high enough (up to $\sqrt{s}=350\,\eV[G]$) to
