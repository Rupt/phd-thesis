...
Variations in rare top processes, which matter in \srllbb, use
...
\begin{itemize}
\item A $\pm30\%$ variation is applied to $\zjets$ in CR-tt to cover our
...
On backgrounds, large uncertainties are conservative, since
flexibility to fit outlying data reduces the chance of mistaken claims of
discoveries.
On signals, however, that effect is reversed.
...
If we want to punish uncertain predictions, then we should consider
likelihoods (not \emph{maximum} \heplikelihood s), as described in
Section~\ref{searches:searces_data_analysis}.
}

In an apparent notional effort to address this problem, we treat the signal
cross-section specially.
Rather than fitting signals cross-section within a constraint, we analyse each
signal model three times with its cross-section fixed to central, up, and down
variations, and report an exclusion contour for each.
These alternative contours have been shown as the three observed limits
in Figures~\ref{fig:2ljets_contours_c1n2} and~\ref{fig:2ljets_contours_gmsb}.
This approach is entirely standard, and is ingrained in \histfitter\ with the
special parameter name
\texttt{$(\alpha\_)$SigXSec}~\cite{histfitter2014,histfittergithub}.
