...
For example, the strong interaction includes a parameter $\theta_\mathrm{CP}$ whose
non-zero values would induce $\mathrm{CP}$ violation.
Observations find it very close to zero, suggesting that nature does not use
that freedom, so one promising feature of axion-like models is their
implication that $\theta_\mathrm{CP} = 0$~\cite{
kim2008axions,
thomson2013modern,
martin2017particle
}.
...
Naturalness is opinion and only nature decides what is really natural.
