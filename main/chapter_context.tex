...
by adding four scalar fields (as a complex doublet) with a potential that moves
the ground state of to a gauge-\emph{asymmetric} configuration.
...
Such an argument could motivate this thesis, but the Standard Model is actually
good in its domain.
...
Such imprecise modelling weakens our inferences from experimental results,
anomalous or not,
but does not present much opportunity because it is common to the quantum field
...
neutrinos.
Or their masses could come from other operators without needing the
right-chiral fields~\cite{
thomson2013modern,
chala2021neutrino,
wells2020discovery
}.
This answer is uncertain, and new results from theory or experiment that
...
Answers to these mysteries all lie somewhere between uncertain and unknown,
...
Certain Standard Model structures are quite general and parametrized such
that parts can be tuned to describe nature.
...
sector.
Observations show that it cannot be far from zero, and one promising feature of
axion-like models is their implication that $\theta_\mathrm{CP} = 0$~\cite{
kim2008axions,
thomson2013modern,
martin2017particle
}.
...
are curious, but will remain speculative until they provide
real and novel benefits.
...
But supersymmetry could still participate in a broken condition where the
sparticles are hidden at larger masses.

Any new model must include the observational facts of the Standard Model,
and with supersymmetry the minimum viable product is the
Minimal Supersymmetric Standard Model (MSSM), which adds the school of
new particles whose mass eigenstates are listed in
Table~\ref{tab:context_particles_mssm},
in which all sparticles are marked with tildes $\tilde{~}$.

% fermion boson multiplets
...
These mix into uncertain mass eigenstates.
Gaugino mixtures are the charged charginos $C^\pm_i$ and
neutral neutralinos $N_i$.
From the four complex scalars, three degrees of freedom are eaten by the
weak bosons to leave five massive scalars $H^0$, $A^0$, $H^\pm$ and the
reborn Standard Model Higgs boson $h$.

None of these new particles is yet observed, but \TODO{}
...
P_R = (-1)^{3(B - L) + 2s}
\end{equation}
\TODO{for baryon number $B$, lepton number $L$ and spin $s$.}

Lightest Supersymmetric Particle (LSP)

Gauge Mediated Supersymmetry Breaking (GMSB)

% reproduce arguments for goldsino and gravitino
% spin-$3/2$ parts couples only gravitationally, see another spin-$1/2$ fermion

\TODO{consistent with a $m(h) \lesssim 130\,\eV[G]$~\cite{
...
Particles of the Minimal Supersymmetric Standard Model in their mass
eigenstates~\cite{martin2016primer}, in addition to the Standard Model
particles of Table~\ref{tab:context_particles_sm}.
The Standard Model Higgs boson $h$ (bracketed) is repeated here with its new
pals.
Sparticles are marked with tildes $\tilde{~}$.
All listed bosons are spin-$0$ and all listed fermions are spin-$1/2$.
The charginos $\tilde C^\pm_i$ and neutralinos $\tilde N^i$ are alternatively
labelled $\chargino_i$ and $\neutralino_i$~\cite{atlas2022searches},
and comprise mixtures of the charged and neutral superpartners to gauge fields
respectively.
Each squark and slepton has two versions from mixtures of the left and right
fields.
Locally broken supersymmetry also adds a spin-$3/2$ gravitino $\tilde G$.
}
