...
which is listed in Table~\ref{tab:context_particles_mssm}.
...
\end{tabular}
\caption[%
Particles and sparticles of the Minimal Supersymmetric Standard Model%
]{%
Particles and sparticles of the Minimal Supersymmetric Standard Model
in their mass eigenstates~\cite{martin2016primer}, in addition to the
Standard Model particles of Table~\ref{tab:context_particles_sm}.
...
which adds yet more fields:
a `secluded' sector which breaks the symmetry, and
a `messenger' sector which
communicates the effect back to the observable sector.
...
All models considered in this thesis conserve $R$-parity
\begin{equation}
P_R = (-1)^{3(B - L) + 2s}
\end{equation}
...
This conservation helps to prevent proton decays,
...
% current observations reaching up to $\eV[T]$ scale
% hierarchy problem

Broken supersymmetry does not perfectly resolve the hierarchy problem, but
could still reduce its fine-tuning if the superpartner masses are below the
$\lesssim 1\,\eV[T]$ scale~\cite{martin2016primer}.

the MSSM has the success that it is consistent with the observed
$m(h) \lesssim 135\,\eV[G]$~\cite{
quiros1997constraints,
wells2020discovery
}

None of these hypothetical particles is yet observed, but it remains possible
that they could be invoked to describe future experimental data.
