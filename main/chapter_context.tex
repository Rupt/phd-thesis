...
For example, $\theta_\mathrm{CP}$ is unattractively fine-tuned within its
experimental constraint of $\theta_\mathrm{CP} \lesssim 10^{-11}$, and that
large $11$ in its exponent defies naturalness.


Another example is the hierarchy problem of the Higgs boson mass $m(h)$,
which notes that the energy scale of new heavy particles linearly modifies the
$m(h)$ through direct or indirect couplings.
