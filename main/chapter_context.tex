...
This discussion is therefore oriented towards how the Standard Model could do
better where it already works, and how that remit might expand to increase its
generality.
...
To distinguish these two cases, we choose for them the respective labels
\emph{uncertain} and \emph{unknown}:
the model of neutrino masses is \emph{uncertain},
but the reconciliation quantum field theory with general relativity is
\emph{unknown}.

A problem with the Standard Model is that algorithms to compute its
...
(of quantum field theory) that all serious alternatives also use.

Before introducing supersymmetry, this section briefly review some promising
directions of change for futrue models.
