...
which notes that the energy scale of new heavy particles add quadratically to
$m(h)^2$ through direct or indirect couplings.
With any particles near the Planck scale $\sim 10^{18}\,\eV[G]$, implementing
the observed $m(h) \sim 10^{2}\,\eV[G]$ therefore requires finely-tuned
cancellations between positive and negative additions.
New fermions subtract from $m(h)$ and new scalars add by similar amounts,
so new models can implement this fine-tuning by inserting only matched pairs of
fermions and scalars --- this is the strategy of supersymmetric models.
...
% table of particle content

% explain mixing

% reproduce arguments for Wino-Bino C1 N2
